...
def _(json_object_model_data, pd):
    if json_object_model_data["errors"] is None:
        ...
        _error_msg = "\n - ".join([el["message"] for el in json_object_model_data["errors"]])
...
def _(alt, df_model):
    ...
    _df_model_agg = df_model.groupby('model').agg({
        'neurons': 'sum'
    }).reset_index()
    _df_model_agg = _df_model_agg.sort_values('neurons', ascending=False).reset_index(drop=True)
    _df_model_agg['neuron_share'] = _df_model_agg['neurons'] / _df_model_agg['neurons'].sum() * 100
...
def _(alt, df_model):
    alt.Chart(df_model).mark_line().encode(
        ...
        tooltip=[alt.Tooltip('time:T', title='Time', format='%b %d, %Y'),
