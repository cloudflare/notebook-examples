...
def _(CF_ACCOUNT_TAG, CF_API_TOKEN, HOSTNAME, json, pd, requests):
    ...
    if _total_pages > 1:
        ...
        print(f'Results are incomplete, will fetch the next {min(_total_pages - 1, _MAX_PAGE_REQUESTS - 1)} pages')
...
def _(json_kv_data, kv_info, pd):
    ...
    df_kv = df_kv.merge(kv_info, left_on='kv', right_on='id',
                        how='left')
    ...
    df_kv['share_of_action_requests'] = df_kv['requests'] /\
        df_kv.groupby('action_type')['requests'].transform('sum') * 100
    return (df_kv,)
...
def _(alt, datetime, df_kv, end_dt, start_dt):
    ...
    _top_reads = df_kv.loc[df_kv['action_type'] == 'read'].groupby('title').agg({
        'requests': 'sum'
    }).reset_index()
    _top_reads = _top_reads.sort_values('requests', ascending=False)

    # Entries not in top 5 are grouped
    _top_reads.loc[~_top_reads['title'].isin(_top_reads.head(_TOP_N)['title']), 'title'] = 'Other'
    _top_reads = _top_reads.groupby('title').agg({
        'requests': 'sum'
    ...
    alt.Chart(_top_reads).mark_bar().encode(
        ...
        alt.Y('title', title='KV name',
              sort=alt.EncodingSortField(field='requests', order='ascending'))
    ).properties(title=alt.TitleParams('KVs with most "read" requests',
...
def _(alt, datetime, df_kv, end_dt, start_dt):
    ...
    _top_write = df_kv.loc[df_kv['action_type'] == 'write'].groupby('title').agg({
        'requests': 'sum'
    }).reset_index()
    _top_write = _top_write.sort_values('requests', ascending=False)

    # Entries not in top 5 are grouped
    _top_write.loc[~_top_write['title'].isin(_top_write.head(_TOP_N)['title']), 'title'] = 'Other'
    _top_write = _top_write.groupby('title').agg({
        'requests': 'sum'
    }).reset_index().sort_values('requests', ascending=False)
    ...
    alt.Chart(_top_write).mark_bar().encode(
        ...
        alt.Y('title', title='KV name',
              sort=alt.EncodingSortField(field='requests', order='ascending'))
    ).properties(title=alt.TitleParams('KVs with most "write" requests',
                                       subtitle=f'Data ranges from {_start_str} to {_end_str}'),
                 height=350, width=700)
    return
...
def _(alt, datetime, df_kv, end_dt, start_dt):
    ...
    # KVs with most "read" requests
    _top_lists = df_kv.loc[df_kv['action_type'] == 'list'].groupby('title').agg({
        'requests': 'sum'
    }).reset_index()
    _top_lists = _top_lists.sort_values('requests', ascending=False)

    # Entries not in top 5 are grouped
    _top_lists.loc[~_top_lists['title'].isin(_top_lists.head(_TOP_N)['title']), 'title'] = 'Other'
    _top_lists = _top_lists.groupby('title').agg({
        'requests': 'sum'
    }).reset_index().sort_values('requests', ascending=False)
    ...
    alt.Chart(_top_lists).mark_bar().encode(
        ...
        alt.Y('title', title='KV name',
              sort=alt.EncodingSortField(field='requests', order='ascending'))
    ).properties(title=alt.TitleParams('KVs with most "list" requests',
                                       subtitle=f'Data ranges from {_start_str} to {_end_str}'),
                 height=350, width=700)
    return
